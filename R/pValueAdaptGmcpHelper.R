...
getICIndex<- function(mcpObj)
{
  ...
  interIndex <-lapply(1:nrow(ClosedIndex), function(x){ClosedIndex[x,]*existIndex})

  primaryIDX <- unlist(apply(ClosedIndex, 1, function(x){paste(x,collapse = '')}))
  correctedIDX <- unlist(lapply(interIndex, function(x){paste(x,collapse = '')}))

  locIDX <- unlist(lapply(correctedIDX,function(x){which(primaryIDX == x)}))
  WH_corr <- cbind(ClosedIndex,
...
do_modifyStrategy <- function(mcpObj, showExistingStrategy = T)
{
  ...
  if(ModificationFlag == 'y')
  {
    ...
    while (cont) {
      ...
      plotGraph(HypothesisName = mcpObj$IndexSet,
...
{
  A <- mcpObj$WH
  allModGraphs <- genWeights(w = mcpObj$newWeights,
                             g = mcpObj$newG,
...
StopTrial <- function(mcpObj)
{
  StopByEfficacy = ifelse(mcpObj$MultipleWinners,
                          ...
                          any(mcpObj$rej_flag_Curr[!mcpObj$DropedFlag]==T)) #if any primary hypothesis is rejected
