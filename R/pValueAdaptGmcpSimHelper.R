...
mnMAMSMEP_sim <- function(gmcpSimObj)
{
  ...
  {
    des <- rpact::getDesignInverseNormal(kMax = K,alpha = gmcpSimObj$alpha,
  ...
  {
    pairs.test <- lapply(names(gmcpSimObj$Arms.Mean)[-1], function(x){c(x,names(gmcpSimObj$Arms.Mean)[1])})
  }else
  {
  ...
  starttime <- Sys.time()
  out<-lapply(1:gmcpSimObj$nSimulation, function(x){SingleSimAnalysis(x, gmcpSimObj,PreSimObj, SimSeed)})
...
SingleSimAnalysis <- function(simID, gmcpSimObj,PreSimObj, SimSeed)
{
  ...
  while(mcpObj$ContTrial)
  {
    ...
    mcpObj$SummStat <- SummStat; mcpObj$ArmData <- ArmData
...
genLookData <- function(simID,mcpObj,PreSimObj,gmcpSimObj,SimSeed)
{
  ...
  testStat <- unlist(lapply(HypoToTest$Names, function(x){pairTestStat(x,df)}))
  ...
  testStat <- cbind(data.frame('SimID'=simID,'LookID'=lookID),
                               testStat,p_values)
...
getArms<- function(SetH,index_map)
{
  ...
  }else
  {
    return(unique(unlist(
      lapply(index_map$Names[index_map$GlobalIndexSet %in% SetH],
...
getPvalBdry <- function(alpha, nLooks, info_frac, typeOfDesign)
{
  ...
  if(UseExternal) #this part of the code can be replaced later with the internal R-codes
  {
    des <- rpact::getDesignInverseNormal(kMax = nLooks,
                                         alpha = alpha,
