...
alpha1 <- rpact::getDesignGroupSequential(
  kMax = 2, alpha = alpha, informationRates = info_frac, typeOfDesign = 'asOF')$alphaSpent[1]
...
trtSS <- sapply(ctrSS, function(x){x*allocRatio[-1]}) #row= trt-arms, col=looks
...
for(l in 1:length(vJ))
{
  for (m in l:length(vJ)) {
    stage2sigmaZ[l,m] <- stage2sigmaZ[m,l] <- 
      varCovZ(i1 = l, k1 = 1,i2 = m, k2 = 1,stage2sigma_0,stage2sigma_trt,
